...
describe("currentWaiverCycle", () => {
  ...
  it("treats Aug 20 as the tail end of the previous cycle", () => {
    expect(
      currentWaiverCycle(Temporal.Instant.from("2025-08-20T23:59:59Z")),
    ).toBe("2024-25");
  });
  ...
  it("crosses year boundaries cleanly", () => {
    expect(
      currentWaiverCycle(Temporal.Instant.from("2026-08-20T12:00:00Z")),
    ).toBe("2025-26");
    expect(
      currentWaiverCycle(Temporal.Instant.from("2026-08-21T00:00:00Z")),
    ).toBe("2026-27");
  });
