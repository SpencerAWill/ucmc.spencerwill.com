...
vi.mock("#/features/auth/server/webauthn.server", async () => {
  const actual = await vi.importActual<typeof WebauthnModule>(
    "#/features/auth/server/webauthn.server",
  );
  return {
    ...actual,
...
const { putChallenge, getChallenge } =
  await import("#/features/auth/server/webauthn-challenge.server");
const {
...
const { insertCredential, listCredentialsForUser } =
  await import("#/features/auth/server/passkey-credentials.server");
