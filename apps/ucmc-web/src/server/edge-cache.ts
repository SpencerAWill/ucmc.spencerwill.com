 ...
 * TanStack Start fetch handler in `apps/ucmc-web/src/server-entry.ts` so
 ...
 * in `apps/ucmc-web/src/config/legal.ts`) or has one that's edited
...
// scheme — see `apps/ucmc-web/src/server/auth/session-cookie.server.ts`
// and `apps/ucmc-web/src/features/auth/server/webauthn-ceremony.server.ts`.
...
//      rest are constants in `apps/ucmc-web/src/config/legal.ts`).
//
