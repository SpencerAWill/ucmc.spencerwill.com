...
- **Web CI** (`web-ci.yml`) — runs on PRs that touch `apps/web/**`, `libs/**`, or the lockfile. Two jobs in parallel: `lint-typecheck-test` (ESLint, TypeScript, Vitest) and `a11y` (axe-core e2e against every public + auth-entry route, gated on serious/critical WCAG 2.1 AA violations per UC EIT 9.2.1).
...
- **Source layout** (Bulletproof React–aligned): the app is organized into four features under `src/features/`:
  - `features/auth/` — sign-in flow (magic link, WebAuthn passkey), session management, the user's own profile/avatar editing, and the `useAuth`/`view-mode`/`guards.ts` "auth public API" surface that other features depend on.
  - `features/members/` — registration approval queue, member directory, admin profile editing, role + permission management (RBAC).
  - `features/announcements/` — announcements feed with admin authoring + read-tracking.
  - `features/feedback/` — in-app bug / feature / general / question submissions with an admin triage view, optionally mirrored to GitHub Issues server-side.
    Each feature has its own `components/`, `api/` (query options + per-mutation `use-*.ts` hooks + `query-keys.ts`), and `server/` (server fns, action implementations, repositories, tests). Shared/foundational code lives outside features: `src/server/auth/` (session, principal, cookies, ids), `src/server/profile/` (zod schemas + constants), `src/components/` (UI primitives + `user-avatar.tsx` + `profile/` field components + `layouts/` + `markdown/` markdown viewer + `editor/` markdown editor), `src/lib/` (utilities), `src/hooks/`, `src/config/`. Routes (`src/routes/`) compose features but never the reverse. The `import/no-restricted-paths` rule in `apps/web/eslint.config.js` enforces these boundaries mechanically — features can't import each other (auth's `api/use-auth.ts`, `api/view-mode.tsx`, and `guards.ts` are the narrow exception), and shared code (`components/ui/`, `components/profile/`, `lib/`, `hooks/`, `config/`) can't import features. Tests are exempt because integration tests legitimately span features.
- **API layer**: every feature follows the same query/mutation pattern in `features/<name>/api/`. `query-keys.ts` centralizes the keys. `queries.ts` exposes `*QueryOptions()` factories that callers spread into `useQuery(...)`. Each mutation has its own `use-*.ts` hook that wraps `useMutation` with a fixed cache-invalidation contract — the call site decides toast/navigate behavior via `mutate(input, { onSuccess, onError })`. Inline `useMutation` calls in routes/components were eliminated in Phase 6 of the Bulletproof refactor.
- **Env**: `@t3-oss/env-core` + zod (`apps/web/src/config/env.ts`) for client (`VITE_*`) vars. Server/Worker vars (APP_BASE_URL, WEBAUTHN_RP_ID, WEBAUTHN_RP_NAME, RESEND_FROM, RESEND_FROM_NAME, RESEND_API_KEY, SESSION_SECRET, MAILPIT_URL) are not in t3-env — they reach request handlers via the Worker `env` binding (`apps/web/src/server/cloudflare-env.ts`). Local dev loads them from `apps/web/.env.local` (copy `.env.example`, gitignored) through wrangler v4's standard `.env` precedence (`.env.<mode>.local` > `.env.local` > `.env.<mode>` > `.env`); `.dev.vars` is no longer used. The devcontainer sets `CLOUDFLARE_INCLUDE_PROCESS_ENV=true` in `docker-compose.yml`, so host-exported env vars (e.g. `SESSION_SECRET`, `RESEND_API_KEY` from the developer's shell) win over `.env.local`. Deployed envs get these same vars from Pulumi (`--var` flags) and GitHub secrets (`wrangler secret put`) via `web-deploy.yml`. Email sending follows a three-tier fallback in `apps/web/src/server/email/resend.ts`: (1) Resend API when `RESEND_API_KEY` is set, (2) Mailpit HTTP send API when `MAILPIT_URL` is set (dev sidecar — emails appear at http://localhost:8025), (3) Worker console log when neither is set.
...
- **Key-value store**: Cloudflare Workers KV via the `KV` binding. Runtime wrapper at `apps/web/src/server/kv/index.ts` exposes `getKv()`. Like D1, KV is **UUID-based** — `wrangler.jsonc` holds placeholder UUIDs (`22222222-...` for dev, `33333333-...` for prod) that the web-deploy workflow rewrites from the Pulumi `kvNamespaceId` stack output before build. Miniflare simulates KV under `apps/web/.wrangler/state/v3/kv/` — no seeding needed. Same "function-wrapped access" invariant. The `/health` route probes D1, R2, KV, and the configured email provider (Resend `/domains`, Mailpit `/api/v1/info`, or console fallback) in parallel.
...
- **`/register/profile` accepts proof OR session**: the magic-link callback sends two distinct user states to `/register/profile` — first-time registrants holding a proof cookie (no `users` row yet) and returning users with a session cookie but no `profiles` row (rare; typically a half-completed prior submit). The `requireRegistrationContext` guard in `src/features/auth/guards.ts` accepts either, exposing `{ source, email }` so the form's read-only email field works the same either way. Anonymous users without proof bounce to `/sign-in?register=true`; signed-in users who already have a profile redirect to `/my/account`. The server's `submitProfileAction` reads either cookie itself, so the contract is symmetric.
...
- **Feedback feature**: `/feedback` lets any approved member (permission `feedback:submit`) post bug reports, feature requests, general feedback, or questions. Submissions are stored in D1 (`feedback` table, schema in `drizzle/schema.ts`) tied to the submitter's `users.id`. Admins (permission `feedback:manage`, granted to `system_admin` via the role bypass) see a triage list on the same page with status changes (`open` → `acknowledged` → `resolved` / `closed`). Each submit also opens a GitHub issue server-side when both `FEEDBACK_GITHUB_TOKEN` (a fine-grained PAT scoped to Issues: read & write) and `FEEDBACK_GITHUB_REPO` (`owner/name`) are set on the Worker env — issue body references only the submitter's opaque `users.publicId`, never their email or full name. The mirror is best-effort: GitHub failures are swallowed and the submission still lands in D1. Rate-limited per user (`feedback:user:<id>` key on the existing `AUTH_RATE_LIMITER` binding — no new wrangler binding needed). Feature lives at `src/features/feedback/`; deploy plumbing pushes the two GitHub vars via `wrangler secret put` in `web-deploy.yml` (env-secret + env-var on each GitHub environment).
- **Multiple verified emails per user**: an account holds ≥1 verified email in the `user_emails` table; one row is `is_primary = 1` (enforced by a partial unique index on `(user_id) WHERE is_primary = 1`). The `users` table no longer has an `email` column — every read site joins `user_emails WHERE is_primary = 1` for the "primary email" projection. Global `UNIQUE(email)` on `user_emails.email` means an address can belong to at most one account; magic-link login looks up the userId via `user_emails`, so any verified address signs into the owning account. Adding an additional email is a magic-link round-trip (`intent = "add_email"`, with `magic_links.target_user_id` holding the requester's userId) gated by `requireApproved()` — pending users keep their single registration email. The consume handler at `/verify-email` asserts `session.userId === magicLink.target_user_id` so a stranger can't request a link to a victim's address and have the victim attach it to the attacker's account by clicking. Race safety relies on the `UNIQUE(email)` constraint via `isUniqueViolation()` in `apps/web/src/server/db/index.ts`, not the pre-check. Removing the primary is blocked (promote another row first); removing the last row is blocked. Promotion is a `db.batch` swap because SQLite enforces partial-unique at statement boundaries. The Principal exposes `primaryEmail: string` and `emails: string[]`. Email normalization (`trim().toLowerCase()`) lives at `apps/web/src/server/auth/email-normalize.ts` — every site that inserts into `user_emails.email` or looks up by it must go through this helper. UI lives as the **"Email addresses" section** on `/my/account/details` (component at `src/features/auth/components/email-addresses-section.tsx`); the verification landing page is `/verify-email`. Audit lifecycle events: `email.added`, `email.removed`, `email.primary_changed`. Test seed pattern: `attachPrimaryEmail(userId, email)` from `#/server/db/test-helpers`.
- **Passkeys (WebAuthn)**: alternative to the magic-link flow, powered by `@simplewebauthn/server` (worker) + `@simplewebauthn/browser` (client). RP config is pinned to `env.WEBAUTHN_RP_ID` / `WEBAUTHN_RP_NAME` / `APP_BASE_URL` (per-env from Pulumi), so credentials are scoped to their host and cannot cross the dev↔prod boundary. Each begin/finish ceremony mints a random ceremony ID, stashes the challenge + intent in KV keyed by `webauthn:ceremony:<id>` with a 5-minute TTL, and sets a short-lived `__Host-ucmc_webauthn` cookie (SameSite=Strict) so the matching finish call can retrieve it. `finish` verifies, deletes the KV entry (single-use), and clears the cookie on every branch. Registered credentials live in the `passkey_credentials` D1 table via `src/features/auth/server/passkey-credentials.server.ts`; the five server fns (register begin/finish, authenticate begin/finish, remove) live in `src/features/auth/server/webauthn-fns.ts`. Authentication uses discoverable credentials (empty `allowCredentials`) so the sign-in page can run a background conditional-UI ceremony — picking a passkey from the browser's autofill menu skips the magic-link send entirely. Magic link stays as the primary registration channel and the fallback recovery channel for lost devices.
- **Testing**: three layers.
  ...
  - **E2e** — Playwright (`apps/web/playwright.config.ts`) drives a real Chromium against a freshly-spawned dev server. The Mailpit sidecar (port 8025) catches outbound mail; the `e2e/fixtures/mailpit.ts` fixture clears the inbox per test and exposes helpers (`waitForMessage`, `extractFirstLink`) so specs can grab magic-link tokens from a live email body. `e2e/fixtures/hydration.ts`'s `waitForHydration(page)` polls TanStack Start's SSR-emitted `window.$_TSR.hydrated` so fills/clicks land after React has reconciled (premature interaction was the source of every flake during the harness build). `e2e/fixtures/db.ts` seeds approved-with-profile users via `wrangler d1 execute` for tests that need an authenticated starting state. The webServer config sets `E2E_BYPASS_RATE_LIMIT=1` (read by `src/server/rate-limit.server.ts`'s `isBypassed()` short-circuit) and clears Turnstile keys, since the rate limiter's 10 req/60 s budget can't cover a full suite run sharing a single IP, and the Turnstile widget polls Cloudflare's CDN in a way that blocks `networkidle` and steals input focus. Run with `pnpm --filter ucmc-web e2e` or `pnpm --filter ucmc-web e2e:ui`. Specs live in `apps/web/e2e/**/*.spec.ts`. Browsers cache lives in the `playwright-browsers` named volume; system libs are baked into the devcontainer Dockerfile and `npx playwright install chromium` runs in `postCreateCommand`. Only `a11y.spec.ts` is wired into CI today (the `a11y` job in `web-ci.yml`); the rest of the e2e suite (magic-link, passkey, register-without-profile) runs locally only.
- **Markdown surface**: a single shared pair of components covers reading + writing markdown across features. `src/components/markdown/markdown-content.tsx` (`<MarkdownContent>`) renders stored bodies via `react-markdown` + `remark-gfm` + `remark-breaks` inside a Tailwind `prose` wrapper; raw HTML pass-through is intentionally NOT enabled (no `rehype-raw`) to keep it XSS-safe, and the `<a>` renderer only sets `target="_blank"`/rel pair on absolute http(s) URLs. `src/components/editor/markdown-editor.tsx` (`<MarkdownEditor>`) is a TipTap-based WYSIWYG that round-trips to a markdown string via `tiptap-markdown` so the storage shape stays a plain text column. The editor is exposed to forms as `field.MarkdownField` (registered in `src/lib/form/form.ts`); the field component lazy-loads the editor through `React.lazy` so the ~265 KB gz TipTap bundle only ships on routes that mount one. Cap enforcement uses the markdown string length (matching what the zod `.max()` validates), placeholder text is read through a ref so changing it doesn't re-create the editor, and the toolbar uses base-ui's `Toolbar` primitive for one-tab-stop arrow-key navigation. Today's only consumer is the feedback feature (`/feedback`) — announcements is the obvious next.
...
- **Compliance content source-of-truth**: `apps/web/src/config/legal.ts` holds every legal/policy string the site renders — the registration disclaimer (UC Rule 40-03-01, font forced via inline style to survive Tailwind purging), sub-brand disambiguation, non-discrimination + anti-hazing + waiver text, privacy/terms/about/membership bodies, the canonical `WAIVER_PDF_PATH` and `WAIVER_VERSION`, and the `POLICIES_VERSION` ack version. The footer's `<RegistrationDisclaimer />` and the `/disclaimer`, `/nondiscrimination`, `/anti-hazing`, `/waiver`, `/privacy`, `/terms`, `/about`, `/membership`, `/legal`, and `/open-source` routes all read from this file. Bumping `WAIVER_VERSION` invalidates every existing waiver attestation (the `requireCurrentWaiver` guard in `apps/web/src/features/auth/guards.ts` filters on `(userId, cycle, version)` together); bumping `POLICIES_VERSION` is informational only — there's no guard that re-prompts for ack. **The legal text on the site must match the canonical PDF served at `WAIVER_PDF_PATH` byte-for-byte; treat copy edits to those routes as legal review, not just word-smithing.**
- **Waiver cycle invariant**: `apps/web/src/config/waiver-cycle.ts` exports `currentWaiverCycle(now)` returning a `"YYYY-YY"` string (e.g. `"2025-26"`). The cycle rolls over on **August 15** of each calendar year (configurable constant `CYCLE_ROLLOVER_MONTH_DAY`); attestations recorded under cycle X automatically stop satisfying `requireCurrentWaiver` once `currentWaiverCycle()` returns X+1. The orchestrator runs in UTC at the worker; the rollover date is calendar-day local-feel (no timezone tracking). Tests pin `now` to control the cycle. **Never compute the cycle ad-hoc** — always import the helper. **The mapping schema for an attestation row is `(userId, cycle, version)` together** — the guard requires all three to match the current world; just having a non-revoked row isn't enough.
- **Compliance roadmap + matrix**: high-level requirements (Ohio law, UC trademark/policy, FERPA, Clery, EIT 9.2.1, constitutional bylaws) live in `.wiki/Compliance.md`, which links each obligation to the file/route that satisfies it. Update that matrix when adding compliance-shaped features (legal pages, retention rules, accessibility gates) so future audits don't have to re-derive the mapping from git history.
...
- **ESLint**: extends the root config, then `@tanstack/eslint-config`, with a few TanStack-specific rules relaxed. `eslint-plugin-jsx-a11y` is wired in with a curated set of rules at error level (alt-text, label-has-associated-control, no-noninteractive-element-interactions, role-has-required-aria-props, etc.) — chosen rather than the plugin's full recommended config so a plugin upgrade can't quietly widen the gate.
...
- **Pulumi** — IaC in `infra/`, TypeScript with the `nodejs` runtime
  ...
  - **Resend domain + sending key** (`infra/resend.ts`) — the custom `ResendDomain` component provisions the Resend sending domain, the Cloudflare DNS records Resend requires (MX return-path, SPF TXT, DKIM), triggers verification, and issues a sending-scoped API key. Implementation uses `@pulumi/command` `local.Command` resources that shell out to `infra/scripts/resend.mjs` (Resend has no Pulumi/Terraform provider, and Pulumi's dynamic providers are incompatible with pnpm — [pulumi/pulumi#9085](https://github.com/pulumi/pulumi/issues/9085)). The script calls the Resend REST API; Pulumi's Cloudflare provider manages the DNS records as first-class resources. **Prerequisite**: a management API key (full*access) must be created manually in the Resend dashboard and stored as a GitHub environment secret (`RESEND_MANAGEMENT_API_KEY`) on both the `dev` and `prod` environments (same pattern as `CLOUDFLARE_API_TOKEN`). For local `pulumi up`, export it in your shell: `export RESEND_MANAGEMENT_API_KEY=re*...`. The sending key is exported as the `resendApiKey`stack output (Pulumi secret) and consumed by`web-deploy.yml`as the Worker's`RESEND_API_KEY`— the GitHub`RESEND_API_KEY`environment secret is no longer used. Component is`protect: true`; destroying it re-issues DKIM keys and invalidates the sending token. **Single-domain sharing**: Resend's free tier only allows one verified domain per account, so the prod stack owns the `ResendDomain`resource (verified on`ucmc.spencerwill.com`) and the dev stack borrows the sending key + from-address via a `pulumi.StackReference`to prod. Behavior is selected by the`resendOwnerStack`Pulumi config: when unset (prod), the stack provisions the resource; when set (dev sets`resendOwnerStack: prod`), the stack reads `resendApiKey`and`resendFromEmail`from the named stack and re-exports them so`web-deploy.yml`keeps working unchanged. Dev emails go out with a`noreply@ucmc.spencerwill.com`envelope — invisible to real recipients and only cosmetically odd in test inboxes. Cross-stack ordering: prod must`pulumi up` before dev so the StackReference outputs exist.
