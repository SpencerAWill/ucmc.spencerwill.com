...
- **Testing**: Vitest with the `@cloudflare/vitest-pool-workers` pool so tests run in a real workerd runtime with Miniflare-simulated D1, KV, and Rate Limiting bindings. Config at `apps/web/vitest.config.ts` loads the drizzle migrations from `drizzle/migrations/` at startup and passes them to the pool as `TEST_MIGRATIONS`; `apps/web/test/apply-migrations.ts` applies them before each file. Tests live next to the code they cover under `src/**/__tests__/*.test.ts`. Cookie helpers (`@tanstack/react-start/server` `getCookie`/`setCookie`) and rate-limit wrappers are `vi.mock`ed in unit tests because they need a request context that tests don't have. Vitest is pinned to 3.2.x and `@cloudflare/vitest-pool-workers` to 0.12.x for compatibility — upgrading vitest to 4.x requires bumping the pool to 0.13+. Testing Library + jsdom are installed but not yet used (no component tests today).
