...
- **Ban (hard ouster)**: distinct from `deactivated`. `status="banned"` + `banned_at`/`banned_by`/`banned_reason` columns + a `banned_emails` blocklist table. Gated by **`members:ban`** (not `members:manage`) so the role gate can diverge later. Three behavioural differences vs. deactivate: (1) magic-link request endpoint **silently drops** banned addresses (timing-pad preserved — `requestMagicLinkAction` early-returns inside the `try`/`finally` so the wall-clock parity with the honored path holds), (2) the `banned_emails` row **survives the user row** via `ON DELETE SET NULL` so self-delete + retention purge cannot free up the address, (3) WebAuthn finish + magic-link consume both refuse banned principals (`reason: "invalid"`) so a passkey enrolled before the ban or a still-valid magic-link minted before the ban cannot open a session. Unban flips status to **`pending`** (re-enters approval queue, **not** approved) and auto-removes that user's blocklist rows in the same step; orphaned (`userId IS NULL`) blocklist rows are intentionally NOT cleared. No auto-purge clock — ban + blocklist are indefinite. Audit: `member.banned` (metadata `{ reason, emailsBlocked }`) → `member.unbanned` (metadata `{ emailsUnblocked }`). UI: 5th tab on `/members/management`, plus a Ban / Unban entry point on the member detail page; the Ban dialog requires reason ≥10 chars.
