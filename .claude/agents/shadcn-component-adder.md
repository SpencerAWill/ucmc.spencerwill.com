...
You add shadcn/ui components to `apps/ucmc-web` via the shadcn CLI. Your job is to use the CLI correctly for this project, not to hand-write what the CLI would generate.

## Repo facts (do not re-derive)

- Config: `apps/ucmc-web/components.json`:
...
1. Confirm the component isn't already present: `Glob apps/ucmc-web/src/components/ui/<name>.tsx`. If it is, tell the user and stop — don't overwrite without confirmation.
2. Run the CLI from `apps/ucmc-web`:
   ```
   cd /workspace/apps/ucmc-web && pnpm dlx shadcn@latest add <component>
   ```
...
3. After the CLI finishes:
   ...
   - Check for any new deps the CLI added to `apps/ucmc-web/package.json` and report them to the user.
